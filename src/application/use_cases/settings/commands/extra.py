...
class ToggleOnboarding(Interactor[None, Optional[SettingsDto]]):
    required_permission = Permission.SETTINGS_EXTRA
    ...
    async def _execute(self, actor: UserDto, data: None) -> Optional[SettingsDto]:
        async with self.uow:
            settings = await self.settings_dao.get()
            settings.extra.onboarding_enabled = not settings.extra.onboarding_enabled
            updated = await self.settings_dao.update(settings)
            await self.uow.commit()

        logger.info(f"{actor.log} Toggled onboarding_enabled: {settings.extra.onboarding_enabled}")
        return updated
