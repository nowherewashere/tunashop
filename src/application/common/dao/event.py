...
class EventsDao(Protocol):
    ...
    the read side backs the offline daily/health jobs. Everything is keyed on
    ``user_ref`` (remnawave_uuid) so bot + site + probe data auto-consolidate.
    """
    ...
    async def node_protocol_health(
        self, *, since: datetime
    ) -> list[HealthRow]:
        """Probe success/total per (node × protocol × operator) since ``since``
        (spec §6.1, §6.5 alert rule)."""
        ...
