...
class ReferralLedgerDao(Protocol):
    ...
    # --- shop-wide totals (admin statistics; same sums without the per-user filter) ---
    ...
    async def get_total_paying_count(self) -> int:
        """Distinct invitees who paid at least once, shop-wide."""
        ...

    async def get_open_payouts_kop(self) -> int:
        """Money locked in the ``requested``/``processing`` payout queue."""
        ...
