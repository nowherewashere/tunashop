...
- **Referral screen (spec §4.7).** «🤝 Приглашай и зарабатывай» with the money‑model value‑props and a live 5‑line stats block (Приглашено / Из них платят / Баланс / Выведено / Потрачено на VPN) — real sums off the ledger, not placeholders. **Вывести** (crypto or Telegram Stars) and **Оплатить VPN балансом** are wired to the payout backend. Two links (bot + a `https://{site}/r/{code}` site link when `REFERRAL_SITE_URL` is set). The copy‑link button is gone; **QR** is blue, **Пригласить** green.
...
**The referral backend is the money model.** Upstream's points/extra‑days accrual is gone (its tables and `users.points` dropped in `0054`): a payment records a commission into `referral_events` at `REFERRAL_RATE_BP`, the balance is derived as `EARNED − SPENT − WITHDRAWN`, and it is cashed out through `payouts` (weekly crypto batch or Telegram Stars) or spent on a subscription via `balance_spends`. Operators settle from **Dashboard → 💸 Выплаты**; **Dashboard → Remnashop → 👥 Реф. система** is the on/off switch plus a read‑only view of the env‑driven parameters. A few spec items are **admin config, not code**: create a single "Standard" plan with 1/3/6/12‑month durations (249/674/1199/2099 ₽); turn off the `device_all_reset` + `link_reset` flags (leaves per‑device unbind); a 72h referred trial = an `INVITED`‑availability trial plan alongside the 24h base.
...
1. **Additive, not invasive.** New behaviour lives in *new files* (new router / use‑case / service packages, config fields, models + DAOs, `.ftl` files, taskiq tasks, event listeners, middleware, numbered migrations from `0041` on). Edits to existing files are single, flag‑guarded lines/blocks. *(One deliberate exception: `0054` drops the superseded upstream reward tables — see the referral note above.)* The retention layer (above) follows the same rule — additive event listeners + tables, no upstream‑model changes.
...
`docker-compose.local.yml` builds the image from source, so your fork's code is what runs. Migrations are applied automatically by `docker-entrypoint.sh` (`alembic upgrade head`) on container start — no manual step. Then open the bot, go to **Dashboard → Remnashop → Extra → Пошаговое подключение**, and turn the funnel on.
...
The money referral program has its own env groups, documented inline in `.env.example`: `REFERRAL_RATE_BP` / `REFERRAL_PAYOUT_MIN_KOP` (commission rate in basis points, crypto payout floor in kopecks), `PAYOUT_*` (mode, crypto asset/network, weekly batch), and `STARS_*` (Telegram Stars payouts: master switch, kopecks‑per‑Star rate, floor, treasury account). The referred‑friend trial length is **not** an env var — it is the first active trial plan with `INVITED` availability.
