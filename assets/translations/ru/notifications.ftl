ntf-error =
    .unknown = ⚠️ <i>Произошла ошибка.</i>
    .permission-denied = ⚠️ <i>У вас недостаточно прав.</i>
    .log-not-found = ⚠️ <i>Лог файл не найден.</i>
    .logs-disabled = ⚠️ <i>Логирование в файл отключено.</i>
    
    .lost-context = ⚠️ <i>Произошла ошибка. Перезапусти диалог командой /start.</i>
    .lost-context-restart = ⚠️ <i>Произошла ошибка. Диалог перезапущен.</i>

ntf-common =
    .trial-unavailable = ⚠️ <i>Пробная подписка временно недоступна.</i>
    .throttling = ⚠️ <i>Слишком много запросов. Подожди немного.</i>
    .double-click-confirm = ⚠️ <i>Нажмите еще раз, чтобы подтвердить действие.</i>
    .squads-empty = ⚠️ <i>Сквады не найдены. Проверьте их наличие в панели.</i>

    .internal-squads-empty = ❌ <i>Выберите хотя бы один внутренний сквад.</i>

    .invalid-value = ❌ <i>Некорректное значение.</i>
    .value-updated = ✅ <i>Параметр успешно обновлен.</i>
    .cooldown-active = ⏳ <i>Временно недоступно. Попробуй снова через { $available_at }.</i>

    .plan-not-found = ❌ <i>План не найден или недоступен.</i>
    .connect-not-available =
    ⚠️ { $status ->
    [LIMITED]
    Ты израсходовал весь доступный трафик. { $is_trial ->
    [0] { $traffic_strategy ->
        [NO_RESET] Продли подписку, чтобы сбросить трафик и снова пользоваться сервисом!
        *[RESET] Трафик восстановится через { $reset_time }. Ещё можно продлить подписку, чтобы сбросить трафик.
        }
    *[1] { $traffic_strategy ->
        [NO_RESET] Оформи подписку, чтобы продолжить пользоваться сервисом!
        *[RESET] Трафик восстановится через { $reset_time }. Ещё можно оформить подписку, чтобы пользоваться сервисом без ограничений.
        }
    }
    [EXPIRED]
    { $is_trial ->
    [0] Срок действия твоей подписки истёк. Продли подписку или оформи новую.
    *[1] Бесплатный пробный период завершён. Оформи подписку, чтобы продолжить пользоваться сервисом.
    }
    *[OTHER] Произошла ошибка при проверке статуса или подписка отключена. Напиши в поддержку.
    }
    
ntf-command =
    .paysupport = 💸 <b>Чтобы запросить возврат, напиши в поддержку.</b>
    .rules = ⚠️ <b>Ознакомься с <a href="{ $url }">Условиями использования</a> перед использованием сервиса.</b>
    .help = 🆘 <b>Нажми кнопку ниже, чтобы связаться с поддержкой.</b>

ntf-requirement =
    .channel-join-required = ❇️ Подпишись на наш канал и получай <b>бесплатные дни, акции и новости</b>. После подписки нажми «Подтвердить».
    .channel-join-required-left = ⚠️ Ты отписался от канала. Подпишись, чтобы продолжить пользоваться ботом.
    .rules-accept-required = ⚠️ <b>Перед использованием сервиса ознакомься и прими <a href="{ $url }">Условия использования</a>.</b>
    .channel-join-error = ⚠️ Мы не видим твою подписку на канал. Проверь подписку и попробуй снова.
    .trial-paused = ⚠️ Пробный период приостановлен — ты отписался от канала. Подпишись снова, чтобы возобновить доступ.
    .trial-restored = ✅ Пробный период возобновлен.
    
ntf-user =
    .not-found = <i>❌ Пользователь не найден.</i>
    .transaction-not-found = ❌ <i>Транзакция не найдена.</i>
    .transactions-empty = ❌ <i>Список транзакций пуст.</i>
    .subscription-empty = ❌ <i>Активная подписка не найдена.</i>
    .subscription-deleted = ✅ <i>Подписка успешно удалена.</i>
    .plans-empty = ❌ <i>Нет доступных планов.</i>
    .devices-empty = ❌ <i>Список устройств пуст.</i>
    .allowed-plans-empty = ❌ <i>Нет доступных планов для предоставления доступа.</i>
    .referral-reset = ✅ <i>Реферальная ссылка успешно сброшена.</i>

    .deleted = ✅ <i>Пользователь удалён полностью: аккаунт, подписки, платежи, рефералы и запись в панели. При следующем /start он зарегистрируется заново.</i>
    .delete-self = ❌ <i>Нельзя удалить самого себя.</i>
    .delete-privileged = ❌ <i>Нельзя удалить пользователя с ролью. Сначала снимите роль.</i>
    .delete-referral-ledger = ❌ <i>У пользователя есть реферальные начисления пригласившему. Удаление списало бы их с чужого баланса — сначала разберитесь с рефералкой.</i>
    .delete-panel-failed = ❌ <i>Не удалось удалить пользователя в панели. Ничего не удалено — попробуйте ещё раз.</i>
    .delete-failed = ❌ <i>Не удалось удалить пользователя.</i>
    .message-success = ✅ <i>Сообщение успешно отправлено.</i>
    .message-failed = ❌ <i>Не удалось отправить сообщение.</i>

    .sync-already = ✅ <i>Данные подписки идентичны.</i>
    .sync-missing-data = ⚠️ <i>Синхронизация невозможна. Данные подписки отсутствуют в панели и в боте.</i>
    .sync-success = ✅ <i>Синхронизация подписки выполнена.</i>

    .invalid-expire-time = ❌ <i>Невозможно { $operation ->
    [ADD] продлить
    *[SUB] сократить
    } срок подписки на указанное количество дней.</i>

ntf-access =
    .maintenance = 🚧 <i>Бот на обслуживании. Попробуй позже.</i>
    .registration-disabled = ❌ <i>Регистрация новых пользователей отключена.</i>
    .registration-invite-only = ❌ <i>Регистрация доступна только по приглашению.</i>
    .payments-disabled = 🚧 <i>Платежи временно недоступны! Пришлём уведомление, когда всё восстановится.</i>
    .payments-restored = ❇️ <i>Платежи восстановлены! Теперь можно купить или продлить подписку. Спасибо за ожидание.</i>

ntf-plan =
    .not-file = ⚠️ <i>Отправьте планы в виде json файла.</i>
    .import-failed = ❌ <i>Не удалось импортировать.</i>
    .import-success = ✅ <i>Успешно импортировано.</i>
    .export-plans-not-selected = ❌ <i>Выберите хотя бы один план для экспорта.</i>
    .export-failed = ❌ <i>Не удалось экспортировать.</i>
    .export-success = ✅ <i>Выбранные планы экспортированы.</i>
    .trial-single-duration = ❌ <i>Пробный план может иметь только одну длительность.</i>
    .duration-already-exists = ❌ <i>Такая длительность уже существует.</i>
    .name-already-exists = ❌ <i>План с таким именем уже существует.</i>
    .user-already-allowed = ❌ <i>Идентификатор пользователя уже добавлен.</i>

    .pool-quota-required = ⚠️ <i>Сначала задайте квоту в гигабайтах.</i>

    .updated = ✅ <i>План успешно обновлен.</i>
    .created = ✅ <i>План успешно создан.</i>
    .deleted = ✅ <i>План успешно удален.</i>

ntf-pool =
    .saved = ✅ <i>Пул сохранён.</i>
    .deleted = ✅ <i>Пул удалён.</i>
    .squad-required = ❌ <i>Выберите сквад пула — без него измерять нечего.</i>
    .squad-not-found = ❌ <i>Такого сквада нет в панели. Обновите список и выберите заново.</i>
    .squad-already-used = ❌ <i>Этот сквад уже занят другим пулом.</i>
    .name-already-exists = ❌ <i>Пул с таким названием уже существует.</i>
    .in-use = ❌ <i>Пул используется в планах. Сначала уберите его квоты из планов.</i>

ntf-gateway =
    .not-configured = ❌ <i>Платежный шлюз не настроен.</i>
    .not-configurable = ❌ <i>У платежного шлюза отсутствуют настройки.</i>
    .test-payment-created = ✅ <i><a href="{ $url }">Тестовый платеж</a> успешно создан.</i>
    .test-payment-error = ❌ <i>Ошибка при создании тестового платежа.</i>
    .test-payment-confirmed = ✅ <i>Тестовый платеж успешно обработан.</i>
    .field-reset = ✅ <i>Значение поля очищено.</i>
    .field-reset-deactivated = ✅ <i>Значение поля очищено. Шлюз отключён: не хватает обязательных настроек.</i>

ntf-subscription =
    .plans-unavailable = ❌ <i>В данный момент нет доступных планов.</i>
    .gateways-unavailable = ❌ <i>В данный момент нет доступных платежных систем.</i>
    .renew-plan-unavailable = ❌ <i>Текущий план устарел и недоступен для продления.</i>
    .payment-creation-failed = ❌ <i>Ошибка при создании платежа. Попробуй позже.</i>

# Показываются всплывающим окном (callback answer): без HTML и не длиннее 200 символов.
ntf-trial-bonus =
    .granted =
        🎁 Готово! Пробный период продлён на { $days } { $days ->
            [one] день
            [few] дня
           *[more] дней
        }. Действует до { $expire_at }.
    .already-claimed = ✅ Этот подарок уже получен — повторно начислить не получится.
    .not-eligible = ❌ Подарок только для пробного периода. У тебя сейчас нет активного или истёкшего пробника.
    .unavailable = ⌛ Предложение больше не действует.

ntf-broadcast =
    .text-too-long = ❌ Превышено максимальное кол-во символов ({ $max_limit }).
    .list-empty = ❌ <i>Список рассылок пуст.</i>
    .plans-unavailable = ❌ <i>Нет доступных планов.</i>
    .audience-unavailable = ❌ <i>Нет пользователей для выбранной аудитории.</i>
    .content-empty = ❌ <i>Контент пуст.</i>
    .content-saved = ✅ <i>Контент успешно сохранен.</i>

    .not-cancelable = ❌ <i>Рассылку невозможно отменить.</i>
    .canceled = ✅ <i>Рассылка успешно отменена.</i>
    .deleting = ⚠️ <i>Выполняется удаление отправленных сообщений.</i>
    .already-deleted = ❌ <i>Рассылка уже удалена или находится в процессе удаления.</i>

    .deleted-success =
        ℹ️ Результат удаления рассылки <code>{ $task_id }</code>.

        <blockquote>
        • <b>Всего сообщений</b>: { $total_count }
        • <b>Удалено</b>: { $deleted_count }
        • <b>Не удалось удалить</b>: { $failed_count }
        </blockquote>

ntf-importer =
    .not-file = ⚠️ <i>Отправьте базу данных в виде файла.</i>
    .db-failed = ❌ <i>Ошибка при экспорте пользователей из базы данных.</i>
    .users-empty = ❌ <i>Список пользователей в базе данных пуст.</i>

    .started = ✅ <i>Импорт запущен. Дождитесь завершения...</i>
    .already-running = ⚠️ <i>Импорт уже выполняется. Пожалуйста, подождите.</i>

ntf-sync =
    .from-panel-started = ✅ <i>Синхронизация панель → бот запущена. Дождитесь завершения...</i>
    .from-bot-started = ✅ <i>Синхронизация бот → панель запущена. Дождитесь завершения...</i>
    .users-not-found = ❌ <i>Пользователи для синхронизации не найдены.</i>
    .already-running = ⚠️ <i>Синхронизация уже выполняется. Пожалуйста, подождите.</i>

ntf-menu-editor =
    .button-saved = ✅ <i>Кнопка успешно сохранена.</i>
    .invalid-payload = ❌ <i>Недопустимый формат URL.</i>

ntf-devices =
    .deleted = ✅ <i>Устройство удалено.</i>
    .all-deleted = ✅ <i>Все устройства удалены.</i>
    .reissued = ✅ <i>Подписка успешно перевыпущена.</i>

ntf-backup =
    .assets-started = ⏳ <i>Создание бэкапа ассетов...</i>
    .db-started = ⏳ <i>Создание бэкапа базы данных...</i>
    .error = ❌ <i>Ошибка при создании бэкапа</i>

ntf-blacklist =
    .list-empty = ❌ <i>Список заблокированных пуст.</i>
    .no-ids-found = ❌ <i>По ссылке не найдено ни одного ID.</i>
    .source-removed = ✅ <i>Список удален.</i>
    .blocked-ids-empty = ❌ <i>Список заблокированных ID пуст.</i>
    .blocked-ids-cleared = ✅ <i>Очищено { $count } ID.</i>
    
    .block-result =
    ℹ️ Результат блокировки.

    <blockquote>
    • <b>Всего ID</b>: { $total }
    • <b>Заблокировано пользователей</b>: { $blocked_users }
    • <b>Заблокировано ID</b>: { $blocked_ids }
    • <b>Уже заблокированные</b>: { $already_blocked }
    </blockquote>

ntf-invite =
    .referral-reset = ✅ <i>Реферальная ссылка обновлена.</i>
    .withdraw-below-min = 💰 Вывод — от { $min } ₽. Не хватает ещё { $remaining } ₽. Приглашай друзей (процент падает на баланс) или оплати балансом свою подписку.
    .payout-requested = ✅ <i>Заявка на вывод { $amount } ₽ принята. Крипта — выплата в ближайший понедельник.</i>
    .payout-requested-stars = ✅ <i>Заявка на Stars принята. Начислим { $stars } ⭐ на твой Telegram в течение дня.</i>
    .payout-below-min = ❌ <i>Недостаточно баланса для вывода в крипту.</i>
    .payout-locked = ⏳ <i>У тебя уже есть заявка на вывод в обработке.</i>
    .payout-negative = ⚠️ <i>Вывод недоступен: баланс отрицательный.</i>
    .payout-bad-wallet = ❌ <i>Проверь адрес кошелька и попробуй снова.</i>
    .wallet-updated = ✅ <i>Адрес вывода обновлён: <code>{ $wallet }</code>.</i>
    .wallet-edit-locked = ⏳ <i>Заявку уже взяли в работу — адрес больше нельзя изменить.</i>
    .stars-no-telegram = ⭐ <i>Чтобы получить Stars, привяжи Telegram. Или выведи на кошелёк.</i>
    .stars-below-min = ❌ <i>Недостаточно баланса для вывода в Stars.</i>
    .stars-unavailable = ⚠️ <i>Вывод в Stars сейчас недоступен.</i>
    .pay-success = ✅ <i>Оплачено с баланса. Подписка продлена на { $days }.</i>
    .pay-insufficient = ❌ <i>Недостаточно баланса для оплаты этого тарифа.</i>
    .pay-no-subscription = ❌ <i>Нет активной подписки для продления. Сначала оформи подписку.</i>
    .pay-failed = ❌ <i>Не удалось оплатить. Попробуй позже.</i>

ntf-promocode =
    .not-found = ❌ <i>Секретный код не найден или недействителен.</i>
    .not-available = ❌ <i>Секретный код недоступен.</i>
    .expired = ❌ <i>Срок действия секретного кода истёк.</i>
    .already-activated = ❌ <i>Ты уже активировал этот секретный код.</i>
    .activated = ✅ <i>Секретный код успешно активирован!</i>
    .activation-failed = ❌ <i>Не удалось активировать секретный код. Попробуй позже.</i>
    .code-exists = ❌ <i>Секретный код с таким значением уже существует.</i>
    .created = ✅ <i>Секретный код создан.</i>
    .deleted = ✅ <i>Секретный код удалён.</i>
    .fields-required = ❌ <i>Заполните значение награды.</i>
    .invalid-code = ❌ <i>Код может содержать только латинские буквы, цифры, дефис и подчёркивание.</i>
    .plans-empty = ❌ <i>Нет доступных планов.</i>
    .updated = ✅ <i>Секретный код обновлён.</i>
    .owner-not-found = ❌ <i>Пользователь не найден. Отправьте @username или Telegram ID.</i>
    .owner-ambiguous = ❌ <i>Найдено несколько пользователей. Уточните запрос — лучше отправьте Telegram ID.</i>
    .owner-set = ✅ <i>Владелец назначен.</i>

ntf-ad-link =
    .created = ✅ <i>Рекламная ссылка создана.</i>
    .updated = ✅ <i>Рекламная ссылка обновлена.</i>
    .deleted = ✅ <i>Рекламная ссылка удалена.</i>
