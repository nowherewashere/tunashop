...
event-pool-traffic =
    .warning =
    <b>⚡ Премиум-локации: осталось немного</b>

    Ты израсходовал <b>{ $percent }%</b> квоты пула <b>{ $pool_name }</b> — { $used } из { $quota }.

    Остальные локации работают без ограничений: трафик там безлимитный, лимит есть только на премиум-нодах. { $has_reset ->
    [1] Квота обновится <b>{ $reset_at }</b>.
    *[0] Квота на премиум-локации не обновляется автоматически — её хватит до конца подписки.
    }

    .exhausted =
    <b>⛔ Премиум-локации отключены — квота израсходована</b>

    Квота пула <b>{ $pool_name }</b> ({ $quota }) израсходована, и премиум-локации отключены. { $has_reset ->
    [1] Они снова включатся <b>{ $reset_at }</b>.
    *[0] На текущем тарифе квота не обновляется автоматически.
    }

    <b>Остальные локации продолжают работать</b> — трафик на них безлимитный. Если премиум нужен прямо сейчас, перейди на тариф с большей квотой: остаток оплаченного срока пересчитается в бонусные дни.
...
event-payment =
    ...
    .referral-failed =
    <b>⚠️ Не удалось начислить реферальную комиссию</b>
    ...
    Покупка завершена успешно, но при начислении реферальной комиссии произошла ошибка. Требуется ручная проверка.
