...
msg-main-menu =
    ...
    *[0]
    ...
    <i>{ $is_trial ->
    [0] Подписка закончилась — верни доступ за пару тапов и снова пробивай блокировки 🐟
    *[1] Пробный период закончился. Оформи подписку — и интернет снова без стен 🐟
    }</i>
    </blockquote>
    [LIMITED]
    <blockquote>
    ...
    *[NONE]
    ...
    <i>{ $trial_available ->
    [1] 🎁 Тебе доступен бесплатный пробный доступ — жми кнопку ниже, подключим за минуту 🐟
    *[0] ↘️ Оформить доступ можно в меню «Подписка».
    }</i>
    </blockquote>
    }
...
msg-menu-invite =
    ...
    <blockquote>
    1️⃣ Получай { $rate }% с <b>КАЖДОГО</b> платежа друзей

    2️⃣ Друзьям — { $trial_days } { $trial_days ->
        [one] день
        [few] дня
       *[more] дней
    } <b>бесплатно</b> и <b>+2 устройства</b> по твоей ссылке

    3️⃣ Выплаты <b>реальными деньгами</b> или <b>оплата подписки балансом</b>
    </blockquote>
...
msg-menu-invite-about =
    ...
    • Друзьям — <b>{ $trial_days } { $trial_days ->
        [one] день
        [few] дня
       *[more] дней
...
msg-extra-onboarding =
    ⚙️ <b>Пошаговое подключение</b>

    Заменяет кнопку «Подключиться» на пошаговый мастер: выбор платформы → установка приложения Happ → добавление конфигурации в один тап → проверка и помощь. Помогает новым пользователям дойти до рабочего подключения.
    ...
    Ссылки на приложения и другие параметры настраиваются через переменные окружения ONBOARDING_*.
...
msg-gateways-methods =
    <b>💳 Методы оплаты Platega</b>

    Отметьте методы, которые пользователь сможет выбрать при оплате через Platega.

    • включено 2 и более — в боте появится выбор метода;
    • включён ровно один — он применится сразу;
    • ни одного — откроется страница выбора Platega.

    Кнопкой «✏️ Название» можно задать своё отображаемое название метода.

msg-gateways-method-label =
    <b>💳 Название метода</b>
...
msg-plan-pool-strategy =
    ...
    Как часто обнуляется счётчик премиум-трафика на этом пуле. Настраивается отдельно от общего лимита трафика плана.

# Traffic pools
msg-pools-main =
    <b>⚡ Пулы трафика</b>

    Пул — это внутренний сквад, содержащий инбаунды только премиум-нод. Расход по нему считается отдельно, а квота задаётся на каждом плане («Планы» → «Сквады» → «Квоты по пулам»).

    Учёт возможен только по ноде: у Xray нет счётчика «пользователь × инбаунд», поэтому премиум-локация обязана быть отдельной нодой.

msg-pool-configurator =
    <b>⚡ Пул: { $name }</b>

    <b>⏺️ Сквад</b>: { $squad }

    Сквад — это и то, что измеряется, и то, что снимается с пользователя при исчерпании квоты.

msg-pool-name =
    <b>🏷️ Название пула</b>

    Пришлите название — его увидят пользователи в кабинете и в уведомлениях (например, «Премиум-локации»).

msg-pool-squad =
    <b>⏺️ Сквад пула</b>

    Выберите внутренний сквад с инбаундами премиум-нод. Сквады, уже занятые другими пулами, в списке не показываются.

msg-pool-nodes =
    <b>🖥 Ноды пула «{ $name }»</b>

    Именно этот трафик будет учитываться квотой:

    { $nodes ->
    [0] <i>Панель не вернула ни одной ноды для этого сквада.</i>
    [GONE] <i>Сквада этого пула больше нет на панели — квота сейчас ничего не измеряет. Выберите сквад заново.</i>
    *[HAS] { $nodes }
    }
...
msg-subscription-plan = 
    ...
    *[HAS]
    ...
    [CHANGE] ℹ️ Текущая подписка будет заменена данным планом; оставшееся время пересчитается в бонусные дни нового тарифа.
    *[OTHER] { empty }
    }
...
msg-subscription-details =
    ...
    *[HAS]
    <blockquote>
    Цены указаны с учётом { $is_personal_discount ->
...
msg-subscription-duration =
    ...
    { $plan_is_modified ->
    [1] ℹ️ Условия плана изменились с момента последней покупки — актуальные данные указаны выше.
    *[0] { "" }
    }
...
msg-promocode-input-owner =
    <b>👤 Владелец (инфлюенсер)</b>
    ...
    *[HAS]
    ...
    Отправьте <b>@username</b> или <b>Telegram ID</b> пользователя-инфлюенсера.

    Активировавшие этот код привяжутся к его реферальной программе — и он будет получать комиссию с их будущих оплат.

    ⚠️ Привязка сработает, только если у пользователя ещё <b>нет</b> реферера: существующий реферер <b>не</b> перезаписывается. Для привязки только новых пользователей выставьте доступ «Для новых».
...
msg-promocode-confirm =
    ...
    { $will_replace_subscription ->
        [1] ⚠️ <i>У тебя уже есть активная подписка. Она будет заменена новым планом; оставшиеся дни сохранятся, трафик обнулится.</i>
       *[0] { space }
    }
